...
Faster RCNNs have Region Proposal Networks that greatly decrease detection timing.

(b) What is Region Proposal Network (RPN)? Clearly explain its architecture. (2)

***Answer:***

A RPN is a fully connected convNet that predicts object bounds and scores at each posistion. The RPN works by running a sliding window through each feature map. At each iteration of the sliding window, anchors are generated with 3 different aspect, ratios and scales. There will be 9 total anchors. 

Each anchor labels the box as either object categories or background. The anchors are invariant of translation, which helps reduce the risk of overfitting. They also use a pyramid of anchors to classify and regress which is makes this step cost effective. Finally, we determine loss by finding boxes that have high intersection rates with our ground truth boxes. 
...
After passing the above image into a CNN block we will recieve a smaller feature map. In the feature block there will be 9 anchors at each posistion, each with a label of either background or a class. Then, these will be further ran into an activation function to predict the labels. 

To avoid large amounts of proposals, the model uses non-maximum
suppression on the proposal regions based on
their respective cls scores. The IoU threshold is fixed to .7 which leaves around 2000 proposal regions.

![image info](sheep.png)
...
For this image we would initially have many proposals. Any proposal with a threshold that is too low gets removed. For example, proposal boxes on the grass and trees would likely get removed as there aren't any objects to be recognized there. But any boxes that remain near the cows or the person would be kept since they are objects we are trying to detect. 
![image info](cows.png)